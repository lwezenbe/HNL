...
if not args.checkLogs:
    ...
    if args.region is None:
        event = Event(sample, new_vars, sample_manager, is_reco_level=not args.genSkim, selection=args.skimSelection, strategy=args.strategy, region=args.region if args.region is not None else 'NoSelection', year = args.year, era = args.era, analysis = args.analysis, obj_sel = obj_sel, additional_options = {'for_skim':True}, ignore_fakerates = True, ignore_weights = True, ignore_searchregions = True)
    else:
        event = Event(sample, new_vars, sample_manager, is_reco_level=not args.genSkim, selection=args.skimSelection, strategy=args.strategy, region=args.region if args.region is not None else 'NoSelection', year = args.year, era = args.era, analysis = args.analysis, ignore_searchregions = True)
