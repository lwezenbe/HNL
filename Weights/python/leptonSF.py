...
class MuonIDSF(LeptonIDSF):
    ...
    def __init__(self, era, year, working_point):
        ...
        if era == 'UL':
            ...
            self.stat_hist = getObjFromFile(root_file_location, 'NUM_LeptonMva{0}_DEN_TrackerMuons_abseta_pt_combined_syst'.format(self.working_point_dict[working_point][era]))
            self.syst_hist = getObjFromFile(root_file_location, 'NUM_LeptonMva{0}_DEN_TrackerMuons_abseta_pt_stat'.format(self.working_point_dict[working_point][era]))
        else:
