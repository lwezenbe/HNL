...
out_file_name = lambda signal_name : os.path.expandvars(os.path.join('$CMSSW_BASE', 'src', 'HNL', 'TMVA', 'data', 'training', args.year, args.region+'-'+args.selection, signal_name, signal_name+'.root'))
	...
	for nt in ntrees:
		for md in maxdepth:
			for bt in boosttypes:
				for s in shrinkage:
					print 'kBDT-boostType={0}-ntrees={1}-maxdepth={2}-shrinkage={3}'.format(bt, nt, md, s)
					factory.BookMethod(loader, ROOT.TMVA.Types.kBDT, 'kBDT-boostType={0}-ntrees={1}-maxdepth={2}-shrinkage={3}'.format(bt, nt, md, s), "NegWeightTreatment=NoNegWeightsInTraining:NTrees={0}:MaxDepth={1}:BoostType={2}:Shrinkage={3}".format(nt, md, bt, s))
