import os
from HNL.Tools.helpers import getObjFromFile, removeNegativeBins
...
def copyEmptyHistogram(to_copy):
    out_hist = to_copy.clone('clone')
    for b in xrange(1, out_hist.getHist().GetNbinsX()+1):
        out_hist.getHist().SetBinContent(b, 0)
        out_hist.getHist().SetBinError(b, 0)
    return out_hist

def loadBackgrounds(input_file, channel):
    backgrounds = []
    valid_hist_for_copying = None
    for b in bkgr_names:
        try:
            #backgrounds.append(Histogram(removeNegativeBins(getObjFromFile(input_file, channel+'_postfit/'+b))))
            backgrounds.append(Histogram(getObjFromFile(input_file, channel+'_postfit/'+b)))
            if valid_hist_for_copying is None:
                valid_hist_for_copying = copyEmptyHistogram(backgrounds[-1])
        except:
            backgrounds.append(None)

    for ib, b in enumerate(bkgr_names):
        if backgrounds[ib] is None:
            backgrounds[ib] = valid_hist_for_copying.clone('clone')
    
    return backgrounds, [x for x in bkgr_names]
...
def trimDataCard(datacard_file):
    with open(datacard_file, 'r+') as fp:
        lines = fp.readlines()
        fp.seek(0)
        fp.truncate()
...
def cleanSyst(syst, bkgr):
    for b in xrange(1, syst.getHist().GetNbinsX()+1):
        tot_bkgr = 0.
        for bk in bkgr:
            tot_bkgr += max(0., bk.getHist().GetBinContent(b))
        syst.getHist().SetBinContent(b, tot_bkgr)
    return syst
...
def loadHistograms(original_folder, channel_names, signal_dict, target_coupling):
    ...
    trimDataCard(new_workspace+'/datacard.txt')
    ...
    syst = cleanSyst(syst, bkgr)

    #import numpy as np
    ##tmp_syst = Histogram(getObjFromFile(input_file, 'ch3_postfit/TT-T+X'))
    #syst.getHist().SetBinError(9, np.sqrt(syst.getHist().GetBinError(8)))
    ...
    if signal_dict is not None:
        ...
        for i, s in enumerate(sorted(signal_dict.keys(), key=sortKey)):
            ...
            if target_coupling is not None: sf = target_coupling / coupling
            ...
            if target_coupling is not None: signal_hist[i].getHist().Scale(sf)
...
def drawPlot(hist_dict, output_name, x_label, x_name, extra_text, name, normalize_bins = False):
    ...
    if x_name != 'Search region': 
        hist_dict = trimDistributions(hist_dict)
    if normalize_bins:
        hist_dict = normalizeBinsAll(hist_dict)

    bkgr = [Histogram(removeNegativeBins(x.getHist())) for x in hist_dict['bkgr_hist']]
    used_bkgr_names = [x for x in hist_dict['bkgr_names']]
    signal = [Histogram(removeNegativeBins(x.getHist())) for x in hist_dict['signal_hist']]
    ...
    p = Plot(signal, signal_legendnames+bkgr_legendnames, name, bkgr_hist = bkgr, observed_hist = observed, draw_ratio = True, year = year, era = era, color_palette = 'HNL', color_palette_bkgr = 'HNLfromTau', y_log=True, y_name='Events', x_name = x_name, extra_text = extra_text, syst_hist = syst_hist, ignore_stat_err = True, for_paper = 'default')
    ...
    p = Plot(signal, signal_legendnames+bkgr_legendnames, name+'_raw', bkgr_hist = bkgr, observed_hist = observed, draw_ratio = True, year = year, era = era, color_palette = 'HNL', color_palette_bkgr = 'HNLfromTau', y_log=True, y_name='Events', x_name = x_name, extra_text = extra_text, syst_hist = syst_hist, ignore_stat_err = True, for_paper = 'raw')
...
def plotFig9p3():
    original_file = os.path.join(input_base, 'Majorana/UL2016post-2016pre-2017-2018/default-lowMassSRloose/e/HNL-e-m60-Vsq2.0em05-prompt/shapes/MVA-lowmass/postfit/NoTau-CD')
    #original_file = os.path.join(input_base, 'Majorana/UL2016post-2016pre-2017-2018/default-lowMassSRloose/e/HNL-e-m30-Vsq4.0em06-prompt/shapes/custom-lowmass/postfit/NoTau-CD')
    #ch1 = EEE-Mu L9-12
    #ch2 = EEE-Mu L13-16
    channels = ['ch1', 'ch2']
...
def plotFig9p4():
    ...
    ##ch8 = MuMuMu-E L13-16
    #channels = ['ch7', 'ch8']
    original_file = os.path.join(input_base, 'Majorana/UL2016post-2016pre-2017-2018/default-lowMassSRloose/mu/HNL-mu-m60-Vsq6.0em06-prompt/shapes/MVA-lowmass/postfit/NoTau-CD')
    #ch7 = MuMuMu-E L9-12
    #ch8 = MuMuMu-E L13-16
    channels = ['ch3', 'ch4']
...
def plotFig10p1():
    ...
    original_file = os.path.join(input_base, 'Majorana/UL2016post-2016pre-2017-2018/default-lowMassSRloose/tau/HNL-tau-m30-Vsq5.0em04-prompt/shapes/MVA-lowestmass/postfit/NoTau-CD')
...
def plotFig10p2():
    ...
    #original_file = os.path.join(input_base, 'Majorana/UL2016post-2016pre-2017-2018/default-lowMassSRloose/tau/HNL-tau-m30-Vsq5.0em04-prompt/shapes/MVA-lowestmass/postfit/SingleTau-CD')
    ##ch3 = TauEE L9-12
    ##ch4 = TauEE L13-16
    ##ch9 = TauMuMu L9-12
    ##ch10 = TauMuMu L13-16
...
def plotFig10p3():
    ...
    ##ch8 = MuMuMu-E L13-16
    #channels = ['ch3', 'ch4', 'ch7', 'ch8']
    original_file = os.path.join(input_base, 'Majorana/UL2016post-2016pre-2017-2018/default-lowMassSRloose/tau/HNL-tau-m60-Vsq2.0em03-prompt/shapes/MVA-lowmass/postfit/NoTau-CD')
    #ch3 = EEE-Mu L9-12
    #ch4 = EEE-Mu L13-16
    #ch7 = MuMuMu-E L9-12
    #ch8 = MuMuMu-E L13-16
...
def plotFig10p4():
    ...
    ##ch10 = TauMuMu L13-16
    #channels = ['ch3', 'ch4', 'ch9', 'ch10']
    original_file = os.path.join(input_base, 'Majorana/UL2016post-2016pre-2017-2018/default-lowMassSRloose/tau/HNL-tau-m60-Vsq2.0em03-prompt/shapes/MVA-lowmass/postfit/SingleTau-CD')
    #ch3 = TauEE L9-12
    #ch4 = TauEE L13-16
    #ch9 = TauMuMu L9-12
    #ch10 = TauMuMu L13-16
...
def plotFig11p3Alternative():
    ...
    extra_text = loadExtraText('eee/ee#mu', 'e', 3e-3)
    drawPlot(hist_dict, output_base+'/PostFitPlotsForPaper/fig11', None, 'BDT(200#minus250, e, 0#tau_{h}) score', extra_text, '11p3alt')
    cleanUp(original_file)



def plotFig8p3Alt():
    original_file = os.path.join(input_base, 'Majorana/UL2016post-2016pre-2017-2018/default-highMassSR/tau/HNL-tau-m200-Vsq4.0em01-prompt/shapes/singlecard/postfit/MaxOneTau')
    #ch1 = EEE-Mu Hb
    #ch2 = EEE-Mu Ha
    #ch3 = MuMuMu-E Hb
    #ch4 = MuMuMu-E Ha
    ...
    signal_files = {
        'HNL-tau-m85-Vsq1.0e+00-prompt' :
            [['default-highMassSR/UL2016post-2016pre-2017-2018/tau/HNL-tau-m85-Vsq1.0e+00-prompt/Majorana/All-EEE-Mu-searchregion.shapes.root', 'All-EEE-Mu/HNL-tau-m85-Vsq1.0e+00-prompt'],
            ['default-highMassSR/UL2016post-2016pre-2017-2018/tau/HNL-tau-m85-Vsq1.0e+00-prompt/Majorana/All-MuMuMu-E-searchregion.shapes.root', 'All-MuMuMu-E/HNL-tau-m85-Vsq1.0e+00-prompt']],
        'HNL-tau-m200-Vsq4.0em01-prompt' :
            [['default-highMassSR/UL2016post-2016pre-2017-2018/tau/HNL-tau-m200-Vsq4.0em01-prompt/Majorana/All-EEE-Mu-searchregion.shapes.root', 'All-EEE-Mu/HNL-tau-m200-Vsq4.0em01-prompt'],
            ['default-highMassSR/UL2016post-2016pre-2017-2018/tau/HNL-tau-m200-Vsq4.0em01-prompt/Majorana/All-MuMuMu-E-searchregion.shapes.root', 'All-MuMuMu-E/HNL-tau-m200-Vsq4.0em01-prompt']],
        'HNL-tau-m300-Vsq7.0em01-prompt' :
            [['default-highMassSR/UL2016post-2016pre-2017-2018/tau/HNL-tau-m300-Vsq7.0em01-prompt/Majorana/All-EEE-Mu-searchregion.shapes.root', 'All-EEE-Mu/HNL-tau-m300-Vsq7.0em01-prompt'],
            ['default-highMassSR/UL2016post-2016pre-2017-2018/tau/HNL-tau-m300-Vsq7.0em01-prompt/Majorana/All-MuMuMu-E-searchregion.shapes.root', 'All-MuMuMu-E/HNL-tau-m300-Vsq7.0em01-prompt']]
    }
    hist_dict = loadHistograms(original_file, channels, signal_files, target_coupling = 4e-1)
    
    extra_text = loadExtraText('0#tau_{h}', 'tau', 4e-1)
    drawPlot(hist_dict, output_base+'/PostFitPlotsForPaper/fig8', ['Ha{0}'.format(x) for x in xrange(1, 26)], 'Search region', extra_text, '8p3', True)
    cleanUp(original_file)

def plotFig8p1p2():
    original_file = os.path.join(input_base, 'Majorana/UL2016post-2016pre-2017-2018/default-lowMassSRloose/tau/HNL-tau-m30-Vsq5.0em04-prompt/shapes/singlecard/postfit/MaxOneTau')
    #ch1 = EEE-Mu L1-4
    #ch2 = EEE-Mu L9-12
    #ch3 = EEE-Mu L5-8
    #ch4 = EEE-Mu L13-16
    ...
    signal_files = {
        'HNL-tau-m20-Vsq5.0em04-prompt' :
            [['default-lowMassSRloose/UL2016post-2016pre-2017-2018/tau/HNL-tau-m20-Vsq5.0em04-prompt/Majorana/All-EEE-Mu-searchregion.shapes.root', 'All-EEE-Mu/HNL-tau-m20-Vsq5.0em04-prompt'],
            ['default-lowMassSRloose/UL2016post-2016pre-2017-2018/tau/HNL-tau-m20-Vsq5.0em04-prompt/Majorana/All-MuMuMu-E-searchregion.shapes.root', 'All-MuMuMu-E/HNL-tau-m20-Vsq5.0em04-prompt']],
        'HNL-tau-m40-Vsq6.0em04-prompt' :
            [['default-lowMassSRloose/UL2016post-2016pre-2017-2018/tau/HNL-tau-m40-Vsq6.0em04-prompt/Majorana/All-EEE-Mu-searchregion.shapes.root', 'All-EEE-Mu/HNL-tau-m40-Vsq6.0em04-prompt'],
            ['default-lowMassSRloose/UL2016post-2016pre-2017-2018/tau/HNL-tau-m40-Vsq6.0em04-prompt/Majorana/All-MuMuMu-E-searchregion.shapes.root', 'All-MuMuMu-E/HNL-tau-m40-Vsq6.0em04-prompt']],
        'HNL-tau-m60-Vsq2.0em03-prompt' :
            [['default-lowMassSRloose/UL2016post-2016pre-2017-2018/tau/HNL-tau-m60-Vsq2.0em03-prompt/Majorana/All-EEE-Mu-searchregion.shapes.root', 'All-EEE-Mu/HNL-tau-m60-Vsq2.0em03-prompt'],
            ['default-lowMassSRloose/UL2016post-2016pre-2017-2018/tau/HNL-tau-m60-Vsq2.0em03-prompt/Majorana/All-MuMuMu-E-searchregion.shapes.root', 'All-MuMuMu-E/HNL-tau-m60-Vsq2.0em03-prompt']]
    }
    hist_dict = loadHistograms(original_file, channels, signal_files, target_coupling = 6e-4)
