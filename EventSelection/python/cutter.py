from HNL.Tools.helpers import isValidRootFile, rootFileContent
...
class Cutter():
    ...
    def cut(self, passed, cut_name):
        ...
        if passed:
            self.list_of_cuts[cut_name].Fill(.5)
            return True
        return False
...
def printCutFlow(in_file_paths, out_file_path, in_file_path_names):
    ...
    out_file.write('NAME \t \t \t ' + '\t'.join([k for k in key_names]))
    for n in in_file_path_names:
        out_file.write(n +'\t \t \t ' + '\t'.join([list_of_cut_values[n][k] for k in key_names]))
    out_file.close()
