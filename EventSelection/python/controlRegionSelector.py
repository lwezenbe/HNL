...
class MuonClosureTest(ClosureTestSelection):
    ...
    def passedFilter(self, chain, new_chain, cutter):
        if not self.initEvent(chain, new_chain, cutter):                                return False
        print chain.l_pt, chain.l_flavor, chain.l_istight, [isFakeLepton(chain, l) for l in chain.l_indices]
        if chain.l_flavor.count(1) == 0:                                                 return False
        print 'next', [chain._lMomPdgId[l] for l in chain.l_indices]
        if not cutter.cut(containsOSSF(chain), 'OSSF present'):                                     return False
        # if not self.signalRegionSelector.passBaseCuts(chain, new_chain, cutter):        return False
        print 'next2'
        if not cutter.cut(self.hasCorrectNumberOfFakes(chain), 'correct number of fakes'): return False
...
# from HNL.ObjectSelection.tauSelector import isJetFakingTau, isTightTau
# from HNL.ObjectSelection.leptonSelector import isTightLepton
# class TauClosureTest(FilterObject):
    
#     def __init__(self, name, selection, objsel, is_reco_level=True, event_categorization = None):
#         super(TauClosureTest, self).__init__(name, selection, objsel, is_reco_level=True, event_categorization = None)
#         self.objsel['tau_wp'] = 'FO'
#         self.objsel['ele_wp'] = 'tight'
#         self.objsel['mu_wp'] = 'tight'
#         self.loose_taus = []
...
#     def hasCorrectNumberOfFakes(self, chain):
#         #Tau will always be the last indices
#         self.loose_taus = []
#         is_tight_tau = []
#         is_fake_tau = []
...
#         #Make input more readable
#         for l in xrange(len(chain.l_pt)):
#             if chain.l_flavor[l] != 2: continue
#             self.loose_taus.append(l)
#             if isTightTau(chain, chain.l_indices[l], algo = self.objsel['tau_algo']): 
#                 is_tight_tau.append(True)
#             else:
#                 is_tight_tau.append(False)
            
#             if isJetFakingTau(chain, chain.l_indices[l]): 
#                 is_fake_tau.append(True)
#             else:
#                 is_fake_tau.append(False)
        
#         #If only one tau present in the event, it is required to be fake
#         if len(self.loose_taus) == 1:
#             return is_fake_tau[0]

#         elif len(self.loose_taus) == 2:

#             #If both taus are tight...
#             if (is_tight_tau[0] and is_tight_tau[1]):
#                 # At least one of the two has to be a fake
#                 return is_fake_tau[0] or is_fake_tau[1]
#             #Otherwise all loose taus should be fake
#             else:
#                 if not is_tight_tau[0] and not is_fake_tau[0]: return False    
#                 if not is_tight_tau[1] and not is_fake_tau[1]: return False 
#                 return True
...
#     def initEvent(self, chain, new_chain, cutter):
#         if not cutter.cut(selectLeptonsGeneral(chain, new_chain, 3, self.objsel, cutter=cutter, sort_leptons = False), 'select leptons'): return False
#         calculateEventVariables(chain, new_chain, is_reco_level=self.is_reco_level, selection=self.selection)
#         chain.category = max(cat.CATEGORIES)
#         return True
...
# from HNL.EventSelection.signalRegionSelector import SignalRegionSelector
# from HNL.ObjectSelection.leptonSelector import isTightLepton
# class ElectronClosureTest(FilterObject):
#     def __init__(self, name, selection, objsel, is_reco_level=True, event_categorization = None):
#         super(ElectronClosureTest, self).__init__(name, selection, objsel, is_reco_level=is_reco_level, event_categorization = event_categorization)
#         self.objsel['tau_wp'] = 'tight'
#         self.objsel['ele_wp'] = 'FO'
#         self.objsel['mu_wp'] = 'tight'
#         signalRegionSelector = SignalRegionSelector('baseline', selection, objsel, is_reco_level=is_reco_level, event_categorization = event_categorization)
#         self.loose_electrons = []

#     def initEvent(self, chain, new_chain, cutter):
#         super(ElectronClosureTest, self).initEvent(chain, new_chain, 3, cutter, sort_leptons = True)
...
#         #Make input more readable
#         for l in xrange(len(chain.l_pt)):
#             if chain.l_flavor[l] != 0: continue
#             self.loose_electrons.append(l)
#             if isTightLepton(chain, chain.l_indices[l], algo = self.objsel['light_algo'], tau_algo = self.objsel['tau_algo']): 
#                 is_tight_electron.append(True)
#             else:
#                 is_tight_electron.append(False)
            
#             if isFakeLepton(chain, chain.l_indices[l]): 
#                 is_fake_electron.append(True)
#             else:
#                 is_fake_electron.append(False)
        
#         #If only one tau present in the event, it is required to be fake
#         if len(self.loose_electrons) == 1:
#             return is_fake_tau[0]

#         elif len(self.loose_electrons) == 2:
...
# class MuonClosureTest(FilterObject):
#     def __init__(self, name, selection, objsel, is_reco_level=True, event_categorization = None):
#         super(MuonClosureTest, self).__init__(name, selection, objsel, is_reco_level=True, event_categorization = None)
#         self.objsel['tau_wp'] = 'tight'
#         self.objsel['ele_wp'] = 'tight'
#         self.objsel['mu_wp'] = 'FO'

#     def initEvent(self, chain, new_chain, cutter):
#         super(MuonClosureTest, self).initEvent(chain, new_chain, 3, cutter, sort_leptons = True)
